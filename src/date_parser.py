...
def process_dataframe(
    ...
    output_column: str = 'FU Date',
    reference_date: Optional[date] = None
) -> pd.DataFrame:
    """
    Process a DataFrame to add/update the FU Date column.
    ...
    Returns:
        DataFrame with new/updated FU Date column
    """
    df = df.copy()
...
def sort_by_follow_up_date(
    ...
    date_column: str = 'FU Date'
) -> pd.DataFrame:
    """
    Sort DataFrame with AFU/ZFU/EFU cases first, ordered by FU Date.

    Replicates the original SQL ordering logic:
    - Cases with TYPE (AFU/ZFU/EFU) come first
    - Within those, sort by FU Date ascending (oldest first)
    - Cases without TYPE come last

    Args:
        df: DataFrame to sort
        type_column: Column containing AFU/ZFU/EFU type
        date_column: Column containing parsed FU dates
    ...
    # Create sort key: 0 for records with TYPE, 1 for others
    df['_has_type'] = df[type_column].notna().apply(lambda x: 0 if x else 1)

    # Sort: first by _has_type, then by FU Date
    df = df.sort_values(
